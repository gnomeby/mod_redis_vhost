...
typedef struct {
	...
	array *ext_mapping;
	array *redis_mapping;
	fcgi_redis_connections *redis_connections;
...
static void fastcgi_site_free(fcgi_site *s) {
	if (!s) return;
...
static fcgi_redis_connections *fastcgi_redis_connections_init() {
	fcgi_redis_connections *f;
...
static void fastcgi_redis_connections_free(fcgi_redis_connections *f)
{
	if (!f) return;
...
static void fastcgi_redis_connection_append(fcgi_redis_connections *redis_connections, redis_connection *rc)
{
	if (redis_connections->size == 0) {
		redis_connections->size = 8;
		redis_connections->connections = malloc(redis_connections->size * sizeof(*(redis_connections->connections)));
		assert(redis_connections->connections);
	} else if (redis_connections->used == redis_connections->size) {
		redis_connections->size += 8;
		redis_connections->connections = realloc(redis_connections->connections, redis_connections->size * sizeof(*(redis_connections->connections)));
		assert(redis_connections->connections);
	}
	redis_connections->connections[redis_connections->used++] = rc;

	return;
}

static fcgi_extension *fastcgi_extension_append(fcgi_exts *ext, buffer *key) {
	fcgi_extension *fe;
	size_t i;
...
static fcgi_site * fastcgi_extension_insert(fcgi_exts *ext, buffer *key, buffer *hostname, fcgi_extension_host *fh) {
	size_t i;
	fcgi_extension * fe;
	fcgi_site * fs;

	fe = fastcgi_extension_append(ext, key);

	for(i = 0; i < fe->used; i++) {
		if (buffer_is_equal(hostname, fe->sites[i]->key)) {
			break;
		}
	}
	...
	if (i == fe->used) {
		...
		fs->key = buffer_init();
		fs->last_used_ndx = -1;
		buffer_copy_string_buffer(fs->key, hostname);

		/* */

		if (fe->size == 0) {
			fe->size = 8;
			fe->sites = malloc(fe->size * sizeof(*(fe->sites)));
			assert(fe->sites);
		} else if (fe->used == fe->size) {
			fe->size += 8;
			fe->sites = realloc(fe->sites, fe->size * sizeof(*(fe->sites)));
			assert(fe->sites);
		}
		fe->sites[fe->used++] = fs;
	} else {
	...
	if (fs->size == 0) {
		fs->size = 4;
		fs->hosts = malloc(fs->size * sizeof(*(fs->hosts)));
		assert(fs->hosts);
	} else if (fs->size == fs->used) {
		fs->size += 4;
		fs->hosts = realloc(fs->hosts, fs->size * sizeof(*(fs->hosts)));
		assert(fs->hosts);
	}
...
FREE_FUNC(mod_fastcgi_free) {
	...
	if (p->config_storage) {
		...
		for (i = 0; i < srv->config_context->used; i++) {
			...
			fastcgi_redis_connections_free(s->redis_connections);
			array_free(s->ext_mapping);
			array_free(s->redis_mapping);
...
SETDEFAULTS_FUNC(mod_fastcgi_set_defaults) {
	...
	config_values_t cv[] = {
		{ "fastcgi.extensions",          NULL, T_CONFIG_ARRAY, T_CONFIG_SCOPE_CONNECTION },       /* 0 */
		...
		{ "fastcgi.redis-servers",       NULL, T_CONFIG_LOCAL, T_CONFIG_SCOPE_CONNECTION },       /* 3 */
	...
	for (i = 0; i < srv->config_context->used; i++) {
		...
		s->redis_mapping = array_init();
		s->redis_connections = fastcgi_redis_connections_init();

		cv[0].destination = s->redis_mapping;
		cv[1].destination = &(s->debug);
		cv[2].destination = s->ext_mapping;
		cv[3].destination = s->redis_connections;
		...
		if (NULL != (du = array_get_element(ca, "fastcgi.redis-servers")))
		{
			size_t j;
			...
			if (du->type != TYPE_ARRAY) {
				log_error_write(srv, __FILE__, __LINE__, "sss",
						"unexpected type for key: ", "fastcgi.redis-servers", "array of strings");
			...
			/*
			 * fastcgi.redis-servers = ( "<ext>" => ( ... ),
			 *          	    	     "<ext>" => ( ... ) )
			 */

			for (j = 0; j < da->value->used; j++) {
				data_array *da_key = (data_array *)da->value->data[j];

				if (da->value->data[j]->type != TYPE_ARRAY) {
					log_error_write(srv, __FILE__, __LINE__, "sssbs",
							"unexpected type for key: ", "fastcgi.redis-servers",
				...
				config_values_t fcv[] = {
					...
					{ "db",         	   NULL, T_CONFIG_SHORT, T_CONFIG_SCOPE_CONNECTION },        /* 2 */
					{ NULL,                NULL, T_CONFIG_UNSET, T_CONFIG_SCOPE_UNSET }
				};

				redis_connection * rc;
				rc = malloc(sizeof(redis_connection));
				...
				buffer_copy_string_buffer(rc->key, da_key->key);
				fcv[0].destination = rc->host;
				fcv[1].destination = &(rc->port);
				fcv[2].destination = &(rc->db);

				if (0 != config_insert_values_internal(srv, da_key->value, fcv)) {
					return HANDLER_ERROR;
				}

				/* Connect to Redis */
				struct timeval timeout = { 1, 500000 }; // 1.5 seconds
				#define FOO(x) (rc->x->used ? rc->x->ptr : NULL)

				rc->context = redisConnectWithTimeout(FOO(host), rc->port, timeout);
				if (rc->context->err) {
					log_error_write(srv, __FILE__, __LINE__, "s", rc->context->errstr);
				}

				fastcgi_redis_connection_append(s->redis_connections, rc);
			}
		}


		if (NULL != (du = array_get_element(ca, "fastcgi.extensions"))) {
			size_t j, k;
			data_array *da = (data_array *)du;

			if (du->type != TYPE_ARRAY) {
				log_error_write(srv, __FILE__, __LINE__, "sss",
						"unexpected type for key: ", "fastcgi.extensions", "array of strings");
			...
			for (j = 0; j < da->value->used; j++) {
				...
				if (da->value->data[j]->type != TYPE_STRING) {
					log_error_write(srv, __FILE__, __LINE__, "sssbs",
							"unexpected type for key: ", "fastcgi.extensions",
							"[", da->value->data[j]->key, "](string)");
				...
				fcgi_extension * fe;
				fe = fastcgi_extension_append(s->exts, da_ext->key);

				// Map redis
				data_string *ds = (data_string *)s->redis_mapping->data[j];
				for (k = 0; k < s->redis_connections->used; k++) {
					if (buffer_is_equal(ds->value, s->redis_connections->connections[k]->key)) {
						fe->redis = s->redis_connections->connections[k];
						break;
					}
...
static handler_t fcgi_write_request(server *srv, handler_ctx *hctx) {
	...
	if (!host->port) {
		log_error_write(srv, __FILE__, __LINE__, "s", "fatal error: neither host->port is not set");
		return HANDLER_ERROR;
	}
...
SUBREQUEST_FUNC(mod_fastcgi_handle_subrequest) {
	...
	if (hctx->host == NULL) {
		...
		ndx = hctx->site->last_used_ndx + 1;
		if(ndx >= (int) hctx->site->used || ndx < 0) ndx = 0;
		host = hctx->site->hosts[ndx];
		if (host->load > 0) {
			/* get backend with the least load. */
			for (k = 0, ndx = -1; k < hctx->site->used; k++) {
				host = hctx->site->hosts[k];
		...
		case -1:
			if (proc->pid && proc->state != PROC_STATE_DIED) {
				...
				default:
					...
					if (p->conf.debug) {
						log_error_write(srv, __FILE__, __LINE__, "ssbsds",
								"--- fastcgi spawning",
								"\n\tsocket", proc->connection_name,
								"\n\tcurrent:", 1, "/");
					}
...
static int fcgi_patch_connection(server *srv, connection *con, plugin_data *p) {
	...
	for (i = 1; i < srv->config_context->used; i++) {
		...
		for (j = 0; j < dc->value->used; j++) {
			...
			if (buffer_is_equal_string(du->key, CONST_STR_LEN("fastcgi.extensions"))) {
				PATCH(exts);
				...
				PATCH(ext_mapping);
			} else if (buffer_is_equal_string(du->key, CONST_STR_LEN("fastcgi.redis-servers"))) {
				PATCH(redis_mapping);
				PATCH(redis_connections);
			}
		}
...
CONNECTION_FUNC(mod_fastcgi_handle_docroot) {
	plugin_data *p = p_d;
	stat_cache_entry *sce;
	...
	if (!p->conf.redis_connections) return HANDLER_GO_ON;
	...
	/* build and run query */
	char * portstr;
	buffer * rrequest, * hostname;
	rrequest = buffer_init();
	hostname = buffer_init();

	portstr = buffer_search_string_len(con->request.http_host, ":", 1);
	if(portstr != NULL)
		buffer_append_string_len(hostname, con->request.http_host->ptr, portstr - con->request.http_host->ptr);
	else
		buffer_append_string_buffer(rrequest, con->request.http_host);

	buffer_append_string(rrequest, "HMGET site:");
	buffer_append_string_buffer(rrequest, hostname);
	buffer_append_string(rrequest, " docroot");

	size_t i;
	for(i = 0; i < p->conf.redis_connections->used; i++)
		if(buffer_is_equal_string(p->conf.redis_connections->connections[i]->key, "docroot", sizeof("docroot") - 1))
			break;
	if(i == p->conf.redis_connections->used) return HANDLER_GO_ON;
	redisContext *c;
	c = p->conf.redis_connections->connections[i]->context;

	redisReply *reply;
    reply = redisCommand(c, rrequest->ptr);
    if(reply->elements == 1 && reply->element[0]->str != NULL)
    {
    	buffer_append_string(docroot, reply->element[0]->str);

    	/* sanity check that really is a directory */
    	BUFFER_APPEND_SLASH(docroot);

    	if (HANDLER_ERROR == stat_cache_get_entry(srv, con, docroot, &sce)) {
    		log_error_write(srv, __FILE__, __LINE__, "sb", strerror(errno), docroot);
    		goto ERR500;
    	}
            if (!S_ISDIR(sce->st.st_mode)) {
    		log_error_write(srv, __FILE__, __LINE__, "sb", "Not a directory", docroot);
    		goto ERR500;
    	}
    ...
    {
		log_error_write(srv, __FILE__, __LINE__, "s", reply->str);
		goto ERR500;
    }
...
GO_ON:
	...
	buffer_copy_string_buffer(con->physical.doc_root, docroot);
...
static fcgi_site * fcgi_findsite(server *srv, connection *con, void *p_d, fcgi_extension *extension)
{
	plugin_data *p = p_d;
	redisReply *reply;
	fcgi_site *site = NULL;
	...
	buffer_append_string(rrequest, "HMGET site:");
	buffer_append_string_buffer(rrequest, hostname);
	buffer_append_string(rrequest, " host port docroot");

    reply = redisCommand(c, rrequest->ptr);
    if(reply->elements == 3 && reply->element[0]->str != NULL && reply->element[1]->str != NULL && reply->element[2]->str != NULL)
    {
    	size_t m;
    	fcgi_site *findsite;
    	buffer * host_id;
    	...
    	host_id = buffer_init();
    	buffer_copy_string(host_id, reply->element[0]->str);
    	buffer_append_string(host_id, ":");
    	buffer_append_string(host_id, reply->element[1]->str);

    	for(m = 0; m < extension->used; m++) {
    		findsite = extension->sites[m];
    		if(buffer_is_equal(host_id, findsite->key))
    			break;
    	}

    	// Create new host and proc
    	if(m == extension->used) {
    		fcgi_extension_host *newhost;

    		newhost = fastcgi_host_init();

			buffer_copy_string_buffer(newhost->id, host_id);

			newhost->check_local  = 1;
			newhost->mode = FCGI_RESPONDER;
			newhost->disable_time = 1;
			newhost->break_scriptfilename_for_php = 0;
			newhost->allow_xsendfile = 0; /* handle X-LIGHTTPD-send-file */
			newhost->fix_root_path_name = 0;

	    	buffer_copy_string(newhost->host, reply->element[0]->str);
	    	sscanf(reply->element[1]->str, "%u", &(newhost->port));
	    	buffer_copy_string(newhost->docroot, reply->element[2]->str);

			if (buffer_is_empty(newhost->host)) {
			...
			proc->id = newhost->num_procs++;
			newhost->max_id++;
			newhost->active_procs++;
			proc->state = PROC_STATE_RUNNING;

			proc->port = newhost->port;

			fastcgi_status_init(srv, p->statuskey, newhost, proc);
			...
			// TODO: ADD TTL
			site = fastcgi_extension_insert(p->conf.exts, extension->key, hostname, newhost);
...
static handler_t fcgi_check_extension(server *srv, connection *con, void *p_d, int uri_path_handler) {
	...
	/* fastcgi.map-extensions maps extensions to existing fastcgi.extensions entries
	 *
	 * fastcgi.map-extensions = ( ".php3" => ".php" )
	 *
	 * fastcgi.extensions = ( ".php" => ... )
	 *
	 * */
	...
	if (extension == NULL) {
		...
		for (k = 0; k < p->conf.exts->used; k++) {
			...
			if (ext->key->ptr[0] == '/') {
				...
				/* check extension in the form ".fcgi" */
				extension = ext;
				break;
			}
		}
	...
	if (extension != NULL) {
		site = fcgi_findsite(srv, con, p, extension);
	}
	if (NULL == site) {
		return HANDLER_GO_ON;
	}
	...
	for (k = 0; k < site->used; k++) {
		fcgi_extension_host *h = site->hosts[k];
...
int mod_fastcgi_plugin_init(plugin *p) {
	...
	p->handle_docroot 		   = mod_fastcgi_handle_docroot;
